...
export type ImageBoxDef = {
  ...
  // 박스 안에 있는 "사진(콘텐츠)" 자체의 절대 위치·크기예요 — box의 xPct/widthPct와
  // 완전히 같은 좌표계를 써요(2026-09-19 변경). 박스(틀)는 이 사진을 비추는 창일
  // 뿐이라, 박스 크기를 조절해도 사진 자체(이 값들)는 바뀌지 않아요 — 창이 좁아지면
  // 더 잘려 보이고 넓어지면 더 보일 뿐이에요(포토샵 클리핑 마스크와 비슷해요). 유일한
  // 예외는 박스를 사진보다 크게 늘렸을 때 빈틈이 생기는 경우로, 그때만 사진을 딱 덮을
  // 만큼 키워요. 지정 안 하면(기존에 저장된 이미지박스) 박스와 같은 값으로 취급해요.
  // lib/imageBoxGeometry.ts가 화면 미리보기·인쇄 파일 둘 다 같은 함수로 계산해요.
  contentXPct?: number;
  contentYPct?: number;
